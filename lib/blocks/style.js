...
function generateStyleImport(descriptor, options) {
    ...
    for (let i = 0; i < styles.length; i++) {
        ...
        if (isCSSModule) {
            const exportAs = style.attribs.module;
            const styleModuleName = exportAs || `style${i}`;
            code += options.esModule
                ? `import ${styleModuleName} from '${resource}';\n`
                : `var ${styleModuleName} = require('${resource}');\n`;
            injectStyles.push({
                exportAs,
                style: styleModuleName
            });
        }
        else {
    ...
    const injectStylesContent = injectStyles.map(function (cur) {
        const exportAs = cur.exportAs === true || !cur.exportAs ? undefined : `'${cur.exportAs}'`;
        return `{exportAs: ${exportAs}, style: ${cur.style}}`;
    }).join(', ');
    code += `var injectStyles = [${injectStylesContent}];\n`;
